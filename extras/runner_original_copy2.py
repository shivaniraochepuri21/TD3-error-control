...
class SimpleRunner:
    ...
    def run(self):
        ...
        for episode in range(self.num_episodes):
            ...
            while not done:
                ...
                reward = self.get_reward(observation, observation_, 0)
    ...
    def test(self, episode):
        ...
        while not done:
            ...
            observation_, reward, done, done_, _ = self.env.step(action)
            reward = self.get_reward(observation, observation_, 1)
    ...
    def test_after_training(self): # we test for one episode
        ...
        while not done:
            action = self.agent.act(observation)
            observation_, reward, done, done_, _ = self.env.step(action)
            reward = self.get_reward(observation, observation_, 2)
    ...
    def get_reward(self, observation, observation_, train_eval_test=0):
        ...
        abs_e = np.linalg.norm(self.e)
        if self.wandb_on:
            if train_eval_test == 0:
                wandb.log({"train_error": abs_e})
            elif train_eval_test == 1:
                wandb.log({"eval_error": abs_e})
            elif train_eval_test == 2:
                wandb.log({"test_error": abs_e})
        return reward
