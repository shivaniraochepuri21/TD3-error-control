...
class MyPendulumEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, render_mode: Optional[str] = None, g=10.0):
        ...
        low = -high
        # high_state = np.array([2*np.pi, self.max_speed], dtype=np.float32)
        # low_state = np.array([0, -self.max_speed], dtype=np.float32)

        # self.observation_space = spaces.Box(low=low_state, high=high_state, shape=(2,), dtype=np.float32)
    ...
    def reset(self, *, init_state = np.array([0.0,0.0]), targ_state = np.array([0.0,0.0]), seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # self.state = np.array([np.deg2rad(359), 0])
        low = -high  # We enforce symmetric limits.
        self.last_u = None
        # low = np.array([0, -self.max_speed])
        
        if init_state.all() == np.array([0.0,0.0]).all(): 
        ...
        # if np.round(self.state[0],3) - np.round(2*np.pi, 3) == 0.000:
            # self.state[0] = 0.0
        if targ_state.all() == np.array([0.0,0.0]).all(): 
            self.targ_state = np.array([0.0, 0.0]) #rad
        else:
